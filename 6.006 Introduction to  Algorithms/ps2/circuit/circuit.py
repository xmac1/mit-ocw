...
class MinHeap(object):
    ...
    def append(self, key):
        ...
        self.__heapFy__(index)
    def __heapFy__(self, index):
        if index == 0:
            return
        parent = self.__parent__(index)
        if self.data[index] < self.data[parent]:
            ...
            self.__heapFy__(index)
        return
    def __sink_down__(self, index):
        smallest = index
        lc = self.__left_child__(index)
        if lc >= len(self.data):
            lc = None
        rc = self.__right_child__(index)
        if rc >= len(self.data):
            rc = None
        if lc == None and rc == None:
            return
        if lc != None and self.data[smallest] > self.data[lc]:
            smallest = lc
        if rc != None and self.data[smallest] > self.data[rc]:
            smallest = rc
        if index != smallest:
            tmp = self.data[index]
            self.data[index] = self.data[smallest]
            self.data[smallest] = tmp
            self.__sink_down__(smallest)
    def min(self):
        if len(self.data) == 0:
            return None
        return self.data[0]

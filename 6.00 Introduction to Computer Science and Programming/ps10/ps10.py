...
def graphRemovedErr(points, kvals = [25, 50, 75, 100, 125, 150], cutoff = 0.1):
    ...
    errHoldouts = []
    for k in kvals:
        partitions = randomPartition(points, 0.2)
        holdout, training = partitions[0], partitions[1]
        clusters, maxDist = kmeans(training, k, cutoff, County)
        error = 0.0
        for cluster in clusters:
            centroid = cluster.getCentroid()
            for point in cluster.getPoints():
                error += centroid.distance(point)
        errors.append(error)
        errHoldout = 0.0
        for point in holdout:
            ...
            for i in range(len(clusters)):
                cluster = clusters[i]
                centorid = cluster.getCentroid()
                dist = centroid.distance(point)
                if dist < closestDist:
                    closestDist = dist
                    index = i
            errHoldout += clusters[index].getCentroid().distance(point)
        errHoldouts.append(errHoldout)
    pylab.plot(kvals, errors, 'b', label='traning set')
    pylab.plot(kvals, errHoldouts, 'b^', label='holdout set')
    pylab.xlabel('number of cluster')
    pylab.ylabel('total error of clusters')
    pylab.title('error of traning and holdout set against k cluster')
    pylab.legend()
    pylab.show()

    ratios = []
    for i in range(len(kvals)):
        ratios.append(errHoldouts[i] / errors[i])
    
    pylab.plot(kvals, ratios)
    pylab.xlabel('number of cluster')
    pylab.ylabel('ratio of holdout set error over traning set error')
    pylab.title('ratio against k')
    pylab.show()

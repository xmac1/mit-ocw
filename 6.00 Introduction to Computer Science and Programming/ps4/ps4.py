...
def build_coder(shift):
    ...
    for letter in upper_letters:
        d[letter] = upper_letters[(i+shift)%len(upper_letters)]
        i += 1
    for letter in lower_letters:
        d[letter] = lower_letters[(i+shift)%len(lower_letters)]
        i += 1
    return d
...
def apply_coder(text, coder):
    ...
    ciper_text = ''
    for letter in text:
        ciper_text += coder.get(letter, letter)
    return ciper_text
...
def apply_shifts(text, shifts):
    ...
    d = {}
    enc_text = text
    for cipher in shifts:
        pos = cipher[0]
        shift = cipher[1]
        enc = apply_shift(enc_text[pos:], shift)
        enc_text = enc_text[:pos] + enc
    return enc_text
...
def find_best_shifts(wordlist, text):
    ...
    """
    l = find_best_shifts_rec(wordlist, text, 0)
    return l
...
def find_best_shifts_rec(wordlist, text, start):
    ...
    for shift in range(0, 27):
        ...
        for letter in text[start:]:
            tmp_str += coder.get(letter, letter)
        cnt = 0
        for word in tmp_str.split(' '):
            if is_word(wordlist, word):
##                print word, 'is a valid word, shift:', shift
                cnt += 1
            else: 
                break
        if cnt >= valid:
            nn = sum([len(word) for word in tmp_str.split(' ')][:cnt]) + cnt
            if nn >= next:
                valid = cnt
                next = nn
                real_shift = shift
    ...
    l = []
    l.append(t)
    plain_text = text[:start]+tmp_text
##    print 'start:', start, 'next:', next, 'shift:', real_shift, 'text:', plain_text
    li = find_best_shifts_rec(wordlist, plain_text, next)
    if li != None:
        l.extend(li)
    return l
